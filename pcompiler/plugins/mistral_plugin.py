"""
Mistral backend plugin — compiles prompts for Mistral models.

Generates the Chat Completions API message format with support for
JSON mode, function calling schemas, and optimized parameters.
"""
...
class MistralPlugin(BackendPlugin):
    """Backend plugin for Mistral models."""
    ...
    def emit(self, ir: PromptIR, profile: ModelProfile) -> CompiledPrompt:
        ...
        if system_parts:
            # Mistral supports system messages, but generally they work 
            # well concatenated with the prompt instructions.
            # We'll put them in a dedicated system message if the model supports it.
